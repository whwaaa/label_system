$(function(){
    		...
    		success: function(data){
    			...
    			if(data.code == 200){
    				...
    				$.each(kuanHaoNameList, function(i, v){
    					var $li = $("<li style='position: relative;'><div class='down-dot'><form action='"+url+"/editLabel/label/download?fileName="+kuanHaoMapList[v][0].typeName+"_"+kuanHaoMapList[v][0].kuanHao+"' method='post' enctype='multipart/form-data'><button class='lnr lnr-download'></button></form></div><p kuanhao='"+kuanHaoMapList[v][0].kuanHao+"' class='sameKuanhao_title'>"+kuanHaoMapList[v][0].typeName+"_"+kuanHaoMapList[v][0].kuanHao+"</p><ul class='sub'></ul></li>")
    					$.each(kuanHaoMapList[v], function(){
    						$li.find(".sub").append($("<div class='subbox'><i class='icon'></i><li id='li_"+this.id+"'>"+this.typeName+"_"+this.kuanHao+"_"+this.color+"_"+this.size+"_"+this.buyPrice+"_"+this.sellPrice+"</li></div>"))	
    					})    
	...
	// 加载标签预览
	function loadLabelBox(kuanhao){
		var i = 1;
		$(".show-box").find(".row").empty()
		$.each(kuanHaoMapList[kuanhao], function(){
			// console.log(this)
			var hidden_styleId = this.templateId
			...
			$(".show-box").find(".row").append($(showbox))
