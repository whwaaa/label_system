$(function(){
	...
	let queryKuanhaoMsgFlag = false;
	let kuanhao;
	// 查询要重录的标签信息
	function queryKuanhaoMsg(){
		let kh = document.location.toString().GetValue("kh");
		$.ajax({
			type: 'GET',
			url: url + "/editLabel/label/" + kuanhao,
			data: {},
			xhrFields: {
				withCredentials: true
			},
			...
			success: function(data){
				layer.closeAll();
				if(data.code == 200){
					let labeldate = data.obj[0];
					// 1.触发模板点击
					$("input[name='templateId"+labeldate.templateId+"']").trigger("click")
					// 2.填充标签数据
					setTimeout(function() {
						$("#zxbz").val(labeldate.zxbz)
						$("#cplb").val(labeldate.cplb)
						$("#mianliao").val(labeldate.outFabric)
						$("#liliao").val(labeldate.intFabric)
						$("#kuanhao").val(labeldate.kuanHao)
						$("#buy_price").val(labeldate.buyPrice)
						$("#sell").val(labeldate.sellPrice)
						$("#guide").val(labeldate.guidePrice)
						$("#color").val(labeldate.color)
						$("#filler").val(labeldate.filler)
						$("input[name='option']").prop("checked",false)
						$.each(data.obj, function(){
							$("#"+this.size).prop("checked",true)
						})
					}, 80);
	...
	function drawingShow(template){
		...
		con += "\t\t\t\t\t<p class=\"p_typeName\" style=\"text-align: center;\">"+template[0].typeName+"</p>\n" +
				...
				"<input class=\"templateId\" name='templateId"+template[0].templateId+"' type=\"hidden\" value=\""+template[0].templateId+"\">" + 
				"\t\t\t\t\t<div class=\"show_box smallLabelWidth\">\n" +
				"\t\t\t\t\t\t<div class=\"label_title\" style=\"margin: 6px auto 6px auto; font-size: 1.5rem;text-align: center;\">合格证</div>\n" +
				"\t\t\t\t\t\t<div class=\"preview\" >\n" +
		...
		if(size==true){
			con += "\t\t\t\t\t\t\t\t<div class=\"pre_size preview_content size visibility\">---</div>";
		}else{
			con += "\t\t\t\t\t\t\t\t<div class=\"pre_size preview_content size visibility\">均码</div>";
		}
		con += "</div>\n" +
				"\t\t\t\t\t\t<div class=\"preview\" >\n" +
		...
		if(guide==true){
			con += "<div class=\"preview\" >\n" +
					...
					"\t\t\t\t\t\t\n" +
					"\t\t\t\t\t\t<div style=\"margin: 2px auto 6px auto; font-size: 1.4rem; text-align: center;\">\n" +
		...
		$.ajax({
			...
			success: function(data){
				if(data.code == 200){
					...
					// 填充
					$("#kuanhao").val(date.substr(0,2)+zimu);
				}
			},
			error: function(){
	...
	function lisen_showbox(){
		$(".showbox").on("click", function(){
			...
			$(".show_box").removeClass(".smallLabelWidth")
			...
			var edit_box = "<div class=\"col-xs-offset-1 col-xs-10 col-md-4\">\n" +
							...
							"\t\t\t\t\t</div>\n" +
							"\t\t\t\t\t<div class=\"oneinput num_option\">\n" +
							"\t\t\t\t\t\t<label >数&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;量</label>\n" +
							"\t\t\t\t\t\t<div style='width:68%;float:right;box-sizing:border-box;margin-top: 5px;'><span style='float:left;margin-right:6px;' class='numoption lnr lnr-circle-minus'></span><input style='max-width:25px;text-align:center;float:left;border:1px solid #D5D5D5;' class='edit_input num_input' type=\"text\" value='1' id=\"number\"><span style='margin-left:6px;' class='numoption lnr lnr-plus-circle'></span><br></div>\n" +
							"\t\t\t\t\t</div>\n" +
							...
							"\t\t\t\t\t\t\t</label> \n" +
							"\t\t\t\t\t\t\t<label class=\"my-checkbox\">\n" +
							"\t\t\t\t\t\t\t\t<input class=\"checkbox\" type=\"checkbox\" name=\"option\" id=\"2XL\" value=\"2XL\"><label for=\"_2XL\">2XL</label>\n" +
							"\t\t\t\t\t\t\t</label> \n" +
							"\t\t\t\t\t\t\t<label class=\"my-checkbox\">\n" +
							"\t\t\t\t\t\t\t\t<input class=\"checkbox\" type=\"checkbox\" name=\"option\" id=\"3XL\" value=\"3XL\"><label for=\"_3XL\">3XL</label>\n" +
							"\t\t\t\t\t\t\t</label> \n" +
							"\t\t\t\t\t\t\t<label class=\"my-checkbox\" style=\"margin-left: 0px;\">\n" +
							"\t\t\t\t\t\t\t\t<input class=\"checkbox\" type=\"checkbox\" name=\"option\" id=\"4XL\" value=\"4XL\"><label for=\"_4XL\">4XL</label>\n" +
							"\t\t\t\t\t\t\t</label> \n" +
							"\t\t\t\t\t\t\t<label class=\"my-checkbox\">\n" +
							"\t\t\t\t\t\t\t\t<input class=\"checkbox\" type=\"checkbox\" name=\"option\" id=\"5XL\" value=\"5XL\"><label for=\"_5XL\">5XL</label>\n" +
							"\t\t\t\t\t\t\t</label> \n" +
							"\t\t\t\t\t\t\t<label class=\"my-checkbox\">\n" +
							"\t\t\t\t\t\t\t\t<input class=\"checkbox\" type=\"checkbox\" name=\"option\" id=\"6XL\" value=\"6XL\"><label for=\"_6XL\">6XL</label>\n" +
							"\t\t\t\t\t\t\t</label>\n" +
							"\t\t\t\t\t\t</div>\n" +
							"\t\t\t\t\t</div>\n" +
							"\t\t\t\t\t<br>\n" +
			...
			if(!size_option){
				...
				$_edit_box.find(".num_option").remove()
			}
			if(!filler_option)
	...
	function inpuptDown(){
		...
		$(".input-box-down").on("click",function(){
			...
			$(this).parents(".input-box").find(".edit_input").trigger("click")
			return false;
		})
	...
	function bindEdit(){
		$(".edit_input").bind('input propertychange click', function() {
			if($(this).hasClass('num_input'))
				return false;
			
			$(this).parents(".input-box").find(".input-box-ul").slideUp(1)
			
			// 颜色文字自动空格
			// if($(this).attr('id').endsWith())
			
			var id = 'pre_' + $(this).attr('id')
			if(id=='pre_buy_price' || id=='pre_sell' || id=='pre_guide'){
				// 进价	   售价	   指导价
				let regx = /^\d+(\.\d{1,2})?$/
			...
			if(id.endsWith("price") || id.endsWith("kuanhao")){
				...
				if(id.endsWith("price")){
					price = $(this).val().replace(".00","");
					let temp = $(".pre_kuanhao").text().substr(3);
					kuanhao = '';
					for(let i=0; i<temp.length; i++){
						if(temp.charAt(i) >= '0' && temp.charAt(i) <= '9'){
							kuanhao += temp.charAt(i);
						}
					}
					$(this).css("border-color", "#ccc");
				}else{ // id.endsWith("kuanhao")
					price = $("#buy_price").val().replace(".00","")
					let temp = $(this).val().substr(3);
					kuanhao = '';
					for(let i=0; i<temp.length; i++){
						if(temp.charAt(i) >= '0' && temp.charAt(i) <= '9'){
							kuanhao += temp.charAt(i);
				...
				if(id.endsWith("sell")){
					if($("#guide").length > 0){// 有指导价
						$('.'+ id).text("工厂直销价 : "+$(this).val().replace(".00","")+".00")
					}else{	// 无指导价
						$('.'+ id).text(" 零售价 : "+$(this).val().replace(".00","")+".00")
					}
				}else if(id.endsWith("guide")){
					$('.' + id).text($(this).val().replace(".00","") + ".00");
				}else{
					$('.' + id).text($(this).val())
				}
				$(this).css("border-color", "#ccc");
	...
	function editNum(){
		$("#number").on('blur', function(){
			...
			if(reg.test(num)){
				...
				$("#number").val(1)
				layer.msg("数量只能填纯数字",function(){})
			}
		})
		$(".lnr-circle-minus").on("click", function(){
			// 数量-1
			var num = $("#number").val()
			var reg = /^[0-9]+$/;     
			if(reg.test(num)){
				if(num > 1){
					...
					$("#number").val(1)
					layer.msg("数量最少为1",function(){})
				}
			}else{
				$("#number").val(1)
			}
		})
		$(".lnr-plus-circle").on("click", function(){
			// 数量+1
			var num = $("#number").val()
			var reg = /^[0-9]+$/;     
			if(reg.test(num))
	...
	function lisenSaveAndCancelBut(){
		$(".save").on("click",function(){
			...
			$.ajax({
				...
				data: {deletekuanhao:kuanhao,typeName:typeName,kuanHao:kuanHao,size:size,color:color,cplb:cplb,
				zxbz:zxbz,outFabric:outFabric,intFabric:intFabric,filler:filler,blank2:$(".hidden_styleId").val(),
				checker:checker,level:level,barCode:barCode,guidePrice:guidePrice,
				sellPrice:sellPrice,buyPrice:buyPrice,templateId:$(".templateId").val(),saveNum:saveNum},
				xhrFields: {
					withCredentials: true
				},
				crossDomain: true,
	...
	function reset(label){
		$(".edit_input").css("border-color", "#ff0000");
		$("#number").css("border-color","#D5D5D5")
		$("#number").val(1)
		if(label != undefined || label != null){
			...
			if(!filler_option)
				$("#filler").text(date.substr(0,2)+zimu)
